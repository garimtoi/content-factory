...
| 카테고리 | 테이블 | 설명 |
|----------|--------|------|
| **Core** | catalogs, subcatalogs, tournaments, events, files, hands, players, hand_players, hand_tags | 콘텐츠 계층 구조 |
...
> **Note**: 기존 `hands.players`, `hands.tags` JSON 컬럼은 하위 호환성을 위해 유지됩니다.
> 동기화 시 JSON 컬럼과 정규화 테이블이 함께 업데이트됩니다.
